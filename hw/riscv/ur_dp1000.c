...
#if 0
#if 0
static bool dp1000_is_acpi_enabled(RISCVVirtState *s)
{
...
static bool dp1000_use_kvm_aia(RISCVVirtState *s)
{
    return kvm_irqchip_in_kernel() && s->aia_type == VIRT_AIA_TYPE_APLIC_IMSIC;
}

static bool dp1000_aclint_allowed(void)
{
    return tcg_enabled() || qtest_enabled();
}

static const MemMapEntry dp1000_memmap[] = {
...
static MemMapEntry dp1000_high_pcie_memmap;

#define VIRT_FLASH_SECTOR_SIZE (256 * KiB)

static PFlashCFI01 *dp1000_flash_create1(RISCVVirtState *s,
                                       const char *name,
...
static void dp1000_flash_create(RISCVVirtState *s)
{
    s->flash[0] = dp1000_flash_create1(s, "virt.flash0", "pflash0");
    s->flash[1] = dp1000_flash_create1(s, "virt.flash1", "pflash1");
}

static void dp1000_flash_map1(PFlashCFI01 *flash,
...
static void dp1000_flash_map(RISCVVirtState *s,
                           MemoryRegion *sysmem)
{
    hwaddr flashsize = dp1000_memmap[VIRT_FLASH].size / 2;
    hwaddr flashbase = dp1000_memmap[VIRT_FLASH].base;

    dp1000_flash_map1(s->flash[0], flashbase, flashsize,
                    sysmem);
    dp1000_flash_map1(s->flash[1], flashbase + flashsize, flashsize,
                    sysmem);
}
                               ...
                               uint32_t *msi_pcie_phandle)
{
    ...
    for (socket = (socket_count - 1); socket >= 0; socket--) {
        ...
        if (dp1000_aclint_allowed() && s->have_aclint) {
    ...
    if (kvm_enabled() && dp1000_use_kvm_aia(s)) {
                            ...
                            uint32_t msi_pcie_phandle)
{
    ...
    qemu_fdt_setprop_sized_cells(ms->fdt, name, "ranges",
        ...
        2, dp1000_high_pcie_memmap.base,
        2, dp1000_high_pcie_memmap.base, 2, dp1000_high_pcie_memmap.size);
...
static void create_fdt_virtio_iommu(RISCVVirtState *s, uint16_t bdf)
{
    ...
    pci_node = g_strdup_printf("/soc/pci@%lx",
                               (long) dp1000_memmap[VIRT_PCIE_ECAM].base);
...
static void finalize_fdt(RISCVVirtState *s)
{
    ...
    create_fdt_sockets(s, dp1000_memmap, &phandle, &irq_mmio_phandle,
                       &irq_pcie_phandle, &irq_virtio_phandle,
                       &msi_pcie_phandle);

    create_fdt_virtio(s, dp1000_memmap, irq_virtio_phandle);

    create_fdt_pcie(s, dp1000_memmap, irq_pcie_phandle, msi_pcie_phandle);

    create_fdt_reset(s, dp1000_memmap, &phandle);

    create_fdt_uart(s, dp1000_memmap, irq_mmio_phandle);

    create_fdt_rtc(s, dp1000_memmap, irq_mmio_phandle);
}
                                          ...
                                          RISCVVirtState *s)
{
    ...
    hwaddr high_mmio_base = dp1000_high_pcie_memmap.base;
    hwaddr high_mmio_size = dp1000_high_pcie_memmap.size;
...
{
    hwaddr base = dp1000_memmap[VIRT_FW_CFG].base;
    FWCfgState *fw_cfg;
...
static DeviceState *dp1000_create_plic(const MemMapEntry *memmap, int socket,
...
static DeviceState *dp1000_create_aia(RISCVVirtAIAType aia_type, int aia_guests,
                                    const MemMapEntry *memmap, int socket,
                                    int base_hartid, int hart_count)
{
    int i;
...
static void create_platform_bus(RISCVVirtState *s, DeviceState *irqchip)
{
    DeviceState *dev;
    SysBusDevice *sysbus;
    const MemMapEntry *memmap = dp1000_memmap;
    int i;
...
static void dp1000_build_smbios(RISCVVirtState *s)
{
...
static void dp1000_machine_done(Notifier *notifier, void *data)
{
    ...
    if (pflash_blk0) {
        if (machine->firmware && !strcmp(machine->firmware, "none") &&
             ...
             */
            start_addr = dp1000_memmap[VIRT_FLASH].base;
        } else {
            /*
            ...
            kernel_entry = dp1000_memmap[VIRT_FLASH].base;
        }
    }
    ...
    riscv_setup_rom_reset_vec(machine, &s->soc[0], start_addr,
                              dp1000_memmap[VIRT_MROM].base,
                              dp1000_memmap[VIRT_MROM].size, kernel_entry,
                              fdt_load_addr);
    ...
    dp1000_build_smbios(s);

    /*
     * Note: dp1000 may NOT need acpi
     */
#if 0
    if (dp1000_is_acpi_enabled(s)) {
        dp1000_acpi_setup(s);
    }
#endif
}

static void dp1000_machine_init(MachineState *machine)
{
    const MemMapEntry *memmap = dp1000_memmap;
    ...
    for (i = 0; i < socket_count; i++) {
        ...
        if (s->aia_type == VIRT_AIA_TYPE_NONE) {
            s->irqchip[i] = dp1000_create_plic(memmap, i,
                                             base_hartid, hart_count);
        } else {
            s->irqchip[i] = dp1000_create_aia(s->aia_type, s->aia_guests,
...
#endif
        dp1000_high_pcie_memmap.base = VIRT32_HIGH_PCIE_MMIO_BASE;
        dp1000_high_pcie_memmap.size = VIRT32_HIGH_PCIE_MMIO_SIZE;
    } else {
        dp1000_high_pcie_memmap.size = VIRT64_HIGH_PCIE_MMIO_SIZE;
        dp1000_high_pcie_memmap.base = memmap[VIRT_DRAM].base + machine->ram_size;
        dp1000_high_pcie_memmap.base =
            ROUND_UP(dp1000_high_pcie_memmap.base, dp1000_high_pcie_memmap.size);
    }

    s->memmap = dp1000_memmap;
    ...
    /* boot rom */
    memory_region_init_rom(mask_rom, NULL, "riscv_dp1000_board.mrom",
    ...
    dp1000_flash_map(s, system_memory);
    ...
    s->machine_done.notify = dp1000_machine_done;
    qemu_add_machine_init_done_notifier(&s->machine_done);
}

static void dp1000_machine_instance_init(Object *obj)
{
    RISCVVirtState *s = RISCV_VIRT_MACHINE(obj);

    dp1000_flash_create(s);
...
static char *dp1000_get_aia_guests(Object *obj, Error **errp)
{
    RISCVVirtState *s = RISCV_VIRT_MACHINE(obj);
...
static void dp1000_set_aia_guests(Object *obj, const char *val, Error **errp)
{
    RISCVVirtState *s = RISCV_VIRT_MACHINE(obj);
...
static char *dp1000_get_aia(Object *obj, Error **errp)
{
    RISCVVirtState *s = RISCV_VIRT_MACHINE(obj);
    const char *val;
...
static void dp1000_set_aia(Object *obj, const char *val, Error **errp)
{
    RISCVVirtState *s = RISCV_VIRT_MACHINE(obj);
...
static bool dp1000_get_aclint(Object *obj, Error **errp)
{
    RISCVVirtState *s = RISCV_VIRT_MACHINE(obj);
...
static void dp1000_set_aclint(Object *obj, bool value, Error **errp)
{
    RISCVVirtState *s = RISCV_VIRT_MACHINE(obj);
...
static void dp1000_get_acpi(Object *obj, Visitor *v, const char *name,
...
static void dp1000_set_acpi(Object *obj, Visitor *v, const char *name,
                          void *opaque, Error **errp)
{
    RISCVVirtState *s = RISCV_VIRT_MACHINE(obj);
...
static HotplugHandler *dp1000_machine_get_hotplug_handler(MachineState *machine,
                                                        DeviceState *dev)
{
...
static void dp1000_machine_device_plug_cb(HotplugHandler *hotplug_dev,
...
static void dp1000_machine_class_init(ObjectClass *oc, void *data)
{
    MachineClass *mc = MACHINE_CLASS(oc);
    HotplugHandlerClass *hc = HOTPLUG_HANDLER_CLASS(oc);

    mc->desc = "RISC-V VirtIO board";
    mc->init = dp1000_machine_init;
    ...
    mc->default_ram_id = "riscv_dp1000_board.ram";
    assert(!mc->get_hotplug_handler);
    mc->get_hotplug_handler = dp1000_machine_get_hotplug_handler;

    hc->plug = dp1000_machine_device_plug_cb;
    ...
    object_class_property_add_bool(oc, "aclint", dp1000_get_aclint,
                                   dp1000_set_aclint);
    ...
    object_class_property_add_str(oc, "aia", dp1000_get_aia,
                                  dp1000_set_aia);
    ...
    object_class_property_add_str(oc, "aia-guests",
                                  dp1000_get_aia_guests,
                                  dp1000_set_aia_guests);
    {
        g_autofree char *str =
    ...
    object_class_property_add(oc, "acpi", "OnOffAuto",
                              dp1000_get_acpi, dp1000_set_acpi,
                              NULL, NULL);
    object_class_property_set_description(oc, "acpi",
                                          "Enable ACPI");
}

static const TypeInfo dp1000_machine_typeinfo = {
    .name       = MACHINE_TYPE_NAME("ur_dp1000"),
    .parent     = TYPE_MACHINE,
    .class_init = dp1000_machine_class_init,
    .instance_init = dp1000_machine_instance_init,
...
static void dp1000_machine_init_register_types(void)
{
    //type_register_static(&dp1000_machine_typeinfo);
    printf("%s: Reach here!\n", __func__);
    exit(-1);
}

type_init(dp1000_machine_init_register_types)
